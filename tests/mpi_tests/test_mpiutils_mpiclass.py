...
# TODO: Remove these as based on defunct functions.

# def test_check_split_ndarray():
#     mpi = MPI()
#     NDshape = [8, 8, 8]
#     array = [np.ones(NDshape[0]), np.ones(NDshape[1]), np.ones(NDshape[2])]
#     split_array = mpi.create_split_ndarray(array, [False, True, True])
#     assert len(split_array[0]) != NDshape[0] and len(split_array[1]) == NDshape[1] and len(split_array[2]) == NDshape[2], "Shape does not match expectation."
#     mpi = MPI()
#     NDshape = [17, 21, 15]
#     array = [np.ones(NDshape[0]), np.ones(NDshape[1]), np.ones(NDshape[2])]
#     split_array = mpi.create_split_ndarray(array, [False, True, True])
#     assert len(split_array[0]) != NDshape[0] and len(split_array[1]) == NDshape[1] and len(split_array[2]) == NDshape[2], "Shape does not match expectation."
#     split_grid = mpi.create_split_ndgrid(array, [False, True, True])
#     print(np.shape(split_grid))
#     assert np.shape(split_grid)[0] != NDshape[0] , "Shape does not match expectation."
#     assert np.shape(split_grid)[1] == NDshape[1], "Shape does not match expectation."
#     assert np.shape(split_grid)[2] == NDshape[2], "Shape does not match expectation."

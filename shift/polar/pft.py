...
class PFT:
        ...
        for i in range(0, int(lenm/2)+1):
            ...
            if self.boundary == 'zero':
                if abs(mval) < mswitch:
                    ...
                    _xnm = bessel.get_Jm_large_zeros(mval, nval, nstop=nswitch, xnm2=_xnm2, xnm1=_xnm1)
                ...
                if abs(mval) < mswitch:
                    ...
                    _xnm1 = self.xnm[abs(mval)-1]
                    _xnm2 = self.xnm[abs(mval)-2]
                    _xnm = bessel.get_dJm_large_zeros(mval, nval, nstop=nswitch, xnm2=_xnm2, xnm1=_xnm1)
                _knm = pft_basis.get_knm(_xnm, self.Rmax)
    ...
    def _benchmark_forward(self, f):
        ...
        for i in range(0, len(self.m2d_flat)):
            Psi = pft_basis.get_Psi_star_nm(self.n2d_flat[i], self.m2d_flat[i], self.r2d, self.p2d, self.knm_flat[i], self.Nnm_flat[i])
    ...
    def _half_fft_forward(self, f):
        ...
        r = self.r2d[0]
        for i in range(0, len(self.m2d_flat)):
            Rnm = pft_basis.get_Rnm(r, self.m2d_flat[i], self.knm_flat[i], self.Nnm_flat[i])
    ...
    def _benchmark_backward(self, Pnm):
        ...
        for i in range(0, len(self.m2d_flat)):
            ...
            Psi = pft_basis.get_Psi_nm(self.n2d_flat[i], self.m2d_flat[i], self.r2d, self.p2d, self.knm_flat[i], self.Nnm_flat[i])
            f += Pnm.real[i]*Psi.real - Pnm.imag[i]*Psi.imag
            utils.progress_bar(i, len(self.m2d_flat))
        return f


    def _half_fft_backward(self, Pnm):
        """Computes the polar Fourier coefficients using FFT for the angular components
        which means each coefficient is calculated from a single integral rather than a
        double integral.
